  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <div className="space-y-2">
        <h1 className="text-2xl font-bold">Game Boy games on the TI-84 Plus CE</h1>
        ...
        {rom && header && (
          <Card className="enter text-sm space-y-2">
            <div className="grid sm:grid-cols-2 gap-x-6 gap-y-1">
              <div><span className="text-slate-400">File:</span> <span className="font-mono">{rom.filename}</span></div>
              <div><span className="text-slate-400">Size:</span> <span className="tabular-nums">{formatBytes(rom.bytes.length)}</span></div>
              <div><span className="text-slate-400">Header title:</span> {header.title || '(none)'}</div>
              <div><span className="text-slate-400">System:</span> {header.cgbOnly ? 'Game Boy Color only' : header.cgb ? 'Game Boy Color (also runs on Game Boy)' : 'Game Boy'}</div>
              <div><span className="text-slate-400">Cartridge:</span> {header.cartridgeTypeName}</div>
              <div><span className="text-slate-400">Save RAM:</span> {header.ramSize ? formatBytes(header.ramSize) : 'none'}</div>
            </div>
            {warnings.map((w) => <p key={w} className="text-amber-200">{w}</p>)}
            {rom.planError && <p className="text-rose-300">{rom.planError}</p>}
          </Card>
        )}
      </section>
          ...
          <Card className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <label className="block text-sm space-y-1">
                <span className="text-slate-300">Name on the calculator (up to 5 characters)</span>
                ...
                <span className={`block text-xs ${prefixError ? 'text-rose-300' : 'text-slate-500'}`}>{prefixError ?? `Creates ${converted ? converted.entries.length : 'the'} AppVars named ${prefix}, ${prefix}R00, ...`}</span>
              </label>
              <label className="block text-sm space-y-1">
                <span className="text-slate-300">Title shown in the emulator's list</span>
                <input className="input w-full" value={title} maxLength={40} onChange={(e) => setTitle(e.target.value)} />
                <span className="block text-xs text-slate-500">Letters, digits and punctuation. Anything else becomes ?.</span>
              </label>
            </div>
          </Card>
        </section>
      ...
      {converted && (
        <section className="enter space-y-3">
          ...
          <Card className="space-y-3 text-sm">
            ...
            {sameNameOnCalc.length > 0 && <p className="text-amber-200">{sameNameOnCalc.join(', ')} already {sameNameOnCalc.length === 1 ? 'exists' : 'exist'} on the calculator and will be replaced.</p>}
            {notEnoughSpace && <p className="text-rose-300">Not enough archive space. Delete something on the My calculator page first.</p>}
            ...
            {sending && (
              <div className="enter space-y-1">
                ...
                <p className="text-xs text-slate-400 tabular-nums">{progress!.index + 1} of {progress!.total} files, {Math.round((100 * progress!.sent) / Math.max(1, progress!.size))}% of the current one</p>
              </div>
            )}
            {done && <Notice>{done}</Notice>}
            <p className="text-xs text-slate-500">The zip holds the same files for TI Connect CE: send all of them to the archive.</p>
          </Card>
        </section>
      )}

      <Card className="text-sm text-slate-300 space-y-2">
        <h2 className="font-semibold text-white">Controls in the emulator</h2>
        <div className="grid sm:grid-cols-2 gap-x-6 gap-y-1">
        ...
        <p className="text-slate-400">Choose a game in the ROM list with the arrow keys and start it with <b>2nd</b> or <b>enter</b>. Save files (battery saves) are kept in an AppVar named after the game plus SAV. Sound and link cable are not emulated.</p>
      </Card>

      <Card className="text-sm text-slate-300 space-y-2">
        <h2 className="font-semibold text-white">About ROM files</h2>
